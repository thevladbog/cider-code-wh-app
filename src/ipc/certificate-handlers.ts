  ...
  ipcMain.handle('certificate:upload', async (_, options: UploadCertificateOptions) => {
    try {
      ...
      if (!options.certData.includes('-----BEGIN CERTIFICATE-----')) {
        console.error('[IPC] Неверный формат данных сертификата - отсутствует маркер BEGIN CERTIFICATE');
        throw new Error('Неверный формат файла сертификата');
      }

      // Проверка формата приватного ключа
      if (!(options.keyData.includes('-----BEGIN PRIVATE KEY-----') || 
            options.keyData.includes('-----BEGIN RSA PRIVATE KEY-----'))) {
        console.error('[IPC] Неверный формат данных приватного ключа - отсутствует маркер BEGIN PRIVATE KEY');
      ...
      if (options.caData) {
        ...
        if (!options.caData.includes('-----BEGIN CERTIFICATE-----')) {
          console.warn('[IPC] CA сертификат не содержит маркер BEGIN CERTIFICATE. Проверьте формат.');
        }
      ...
      if (!certInfo.valid) {
        ...
        if (!certInfo.domain) {
          ...
          console.error('[IPC] Отсутствует информация о домене. Попробуйте использовать сертификат с указанным CN или SAN полями.');
      ...
      try {
        ...
        const verifyPromise = new Promise<boolean>((resolve, reject) => {
          execFile('node', [script, certFile, keyFile], (error: Error | null, stdout: string, stderr: string) => {
            if (error) {
