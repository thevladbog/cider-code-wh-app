...
const ModalOrderSearch: React.FC<ModalOrderSearchProps> = ({ isOpen, onClose, onOrderSelect }) => {
  const { archivedOrders } = useStore();
  ...
  const handleSearch = () => {
    const results = archivedOrders.filter(
    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">Печать по № заказа</h2>
          <button aria-label="Закрыть" onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 gap-2 mb-2 select-none">
            {[1,2,3,4,5,6,7,8,9].map(n => (
              <button key={n} className="numpad-button" onClick={() => handleNumpad(n.toString())}>{n}</button>
            ))}
            <button className="numpad-button bg-red-100 dark:bg-red-900" onClick={handleClear}>C</button>
            <button className="numpad-button" onClick={() => handleNumpad('0')}>0</button>
            <button className="numpad-button" onClick={handleBackspace}>⌫</button>
          </div>
          <button
        ...
        {searchResults.length > 1 && (
          <div className="mt-4">
            <div className="mb-2 text-sm text-gray-600 dark:text-gray-300">Найдено заказов: {searchResults.length}</div>
